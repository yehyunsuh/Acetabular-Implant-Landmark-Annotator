...
def draw_best_fit_ellipse(image: np.ndarray,
                          points: List[Tuple[int, int]],
                          img_width: int,
                          color: Tuple[int, int, int] = (0, 255, 255),
                          thickness: int = 2) -> None:
    """
    If > 6 landmarks exist, fit an ellipse (Fitzgibbon) on points inside the image area
    (exclude legend padding) and draw it on `image`. If <= 6, draw nothing.

    Args:
        image (np.ndarray): The padded image currently being displayed (legend on the right).
        points (List[Tuple[int, int]]): Clicked points in *padded* image coordinates.
        img_width (int): Width of the actual image region (before legend padding).
        color (Tuple[int,int,int]): BGR color for the ellipse.
        thickness (int): Line thickness for the ellipse.
    """
    # Use only points *inside* the actual image (ignore clicks in the legend area)
    in_img_pts = [(x, y) for (x, y) in points if x < img_width]
    if len(in_img_pts) <= 6:
        return  # requirement: only draw when more than 6 points

    try:
        # Fit expects [N, 3]; supply z=0
        P = np.zeros((len(in_img_pts), 3), dtype=np.float64)
        P[:, 0] = [p[0] for p in in_img_pts]
        P[:, 1] = [p[1] for p in in_img_pts]

        # Try calling directly with NumPy (if you ported it)
        params = fitzgibbon_et_al(P)  # shape [5,]
        cx, cy, a, b, ang = float(params[0]), float(params[1]), float(params[2]), float(params[3]), float(params[4])
        
        # OpenCV expects semi-axes as the axes tuple, angle in degrees
        center = (int(round(cx)), int(round(cy)))
        axes = (int(round(a)), int(round(b)))  # (semi-major, semi-minor)
        angle = float(ang) + 90

        # Draw the ellipse (full 0–360 degrees)
        cv2.ellipse(image, center, axes, angle, 0, 360, color, thickness, lineType=cv2.LINE_AA)

    except Exception as e:
        # Stay silent in UI; uncomment for debugging:
        # print(f"[Ellipse draw skipped] {e}")
        pass
...
def redraw_image(params: dict) -> None:
    ...
    draw_best_fit_ellipse(padded_image, clicked_points, w)
...
def save_annotation(image_path: str, max_clicks: int, width: int, height: int, radius: int, vis_resize: int, colors: List[Tuple[int, int, int]]) -> None:
    ...
    draw_best_fit_ellipse(padded_image, clicked_points, original_w)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_clicks', type=int, default=10, help='Number of landmarks per image')
